...
import { Play, Pause, RotateCcw, Calendar, Waves } from 'lucide-react';
...
export const TimelineSlider: React.FC<TimelineSliderProps> = ({ onTimeChange, selectedDays }) => {
  ...
  return (
    <motion.div
      ...
      className="absolute bottom-6 left-1/2 -translate-x-1/2 z-[1000] w-[540px] bg-[#07172A]/90 border border-[#0A84FF]/40 backdrop-blur-xl px-5 py-3 rounded-2xl shadow-2xl text-white font-sans"
    >
      <div className="flex items-center justify-between mb-2 text-xs font-mono">
        <div className="flex items-center space-x-2">
          <Waves className="w-4 h-4 text-cyan-400 animate-pulse" />
          <span className="font-bold text-cyan-300">Hydro-Dynamic Drift Simulation</span>
        </div>
        <span className="text-[10px] text-gray-400 bg-white/5 px-2 py-0.5 rounded border border-white/5">
          Temporal Offset: +{selectedDays} Days
        </span>
      </div>
      ...
      <div className="flex items-center space-x-4">
        ...
        <button
          ...
          className="p-2.5 rounded-xl bg-[#0A84FF] hover:bg-cyan-400 text-slate-950 font-bold shadow-[0_0_15px_rgba(10,132,255,0.5)] transition-all cursor-pointer flex items-center justify-center"
        ...
        {/* Step Buttons / Track */}
        <div className="flex-1 flex items-center justify-between relative bg-white/5 p-1.5 rounded-xl border border-white/5">
