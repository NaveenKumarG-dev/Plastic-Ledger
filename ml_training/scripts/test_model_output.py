...
spec = importlib.util.spec_from_file_location(
    "detect_stage",
    Path(__file__).resolve().parent.parent.parent / "src" / "pipeline" / "03_detect.py"
)
detect_module = importlib.util.module_from_spec(spec)
spec.loader.exec_module(detect_module)
load_model = detect_module.load_model

# Load model
model_path = Path('models/segformer/runs/segformer_v1/best_model_SegTransformer.pth')
